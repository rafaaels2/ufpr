...
int mqueue_init (mqueue_t *queue, int max, int size) {
    ...
    queue -> buffer = malloc (sizeof (size) * max);
    queue -> max = max;
    queue -> n_msgs = 0;
...
int mqueue_send (mqueue_t *queue, void *msg) {
    ...
    if (put (queue, msg, queue -> max + 1) < 0) 
        return -1;
