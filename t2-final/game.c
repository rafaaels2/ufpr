...
int values[11] = {52, 53, 54, 55, 81, 74, 75, 65, 50, 51, 48};
int suits[4] = {1, 2, 3, 4};
...
int recv_msg (frame_t *frame, info_t *info, int addr) {
    ...
    if (frame -> type != NOT) {
        ...
        /* verifica se a mensagem eh para o jogador atual */
            ...
            if (info -> lifes[addr] <= 0) {
                if (frame -> type == CARD) {
                    cards[n_card].value = 10;
                    cards[n_card].suit = 0;
                }
...
void run_game (frame_t * frame, info_t *info, int shackle, int addr) {
    ...
    while (info -> n_cards != info -> crr_round) {
        ...
        while (frame -> data.round.n_plays != info -> n_players) {
            /* envia a carta e espera a resposta */
            send_msg (frame, addr);
            recv_msg (frame, info, addr);
            ...
            if (frame -> data.round.n_plays != info -> n_players) {
