...
usage() {
  ...
  Usage:
    ...
    OPTIONS
      ...
      -a ................... append to existing log files
      -b BenchmarkDir ...... use files from this directory for the benchmark.
      -B BenchmarkURLfile .. file with 1 URLs to each benchmark
      ...
      -s stopfile .......... abort benchmarking in case the given file is detected
      -S suspendfile........ halt benchmarking for the given amount of seconds
...
process_solver_output() {
    ...
    local CPUTIME="$(awk -F':' '/\[runlim\] time:/ {print $2}' "$LOG_FILE" | sed 's:seconds::g' | xargs)"
    ...
    if [ -n "$LOG_DUMP" ] && [ ! -r "$LOG_DUMP" ]; then
        echo "benchmark;solver;sat-status;time;cputime;memory;status;runlim-status" >>"$LOG_DUMP"
    fi
    [ -n "$LOG_DUMP" ] && echo "$benchmark;$solver;$SAT_RESULT;$RUNTIME;$CPUTIME;$SPACE;$STATUS;$RUNLIM_STATUS" >>"$LOG_DUMP"
    [ "$VERBOSE" -gt 0 ] && echo "[stats] $benchmark;$solver;$SAT_RESULT;$RUNTIME;$CPUTIME;$SPACE;$STATUS;$RUNLIM_STATUS"
    ...
    if [ "$STATUS" == "out of time" ] || [ "$STATUS" == "out of memory" ]; then
        ...
        TOTALTIME["$solver"]=$(echo "${TOTALTIME["$solver"]} + $RUNTIME" | bc)
        TOTALCPUTIME["$solver"]=$(echo "${TOTALCPUTIME["$solver"]} + $CPUTIME" | bc)
    fi
...
while getopts "ab:B:dDhl:m:o:Rs:S:t:vy" OPTION; do
    case $OPTION in
    a)
        APPEND="-a"
        ;;
...
# create log dump file and directory
if [ -n "$"$LOG_DUMP"" ]; then
...
trap '[ -d "$TMP_WORK_DIR" ] && rm -rf "$TMP_WORK_DIR"' EXIT
TMP_WORK_DIR=$(mktemp -d)
TMP_INPUT_FILE="$TMP_WORK_DIR"/downloaded-cnf.cnf.xz
WGET_LOG_FILE="$TMP_WORK_DIR"/wget.log
...
if [ -z "$RESULTS_DIRS" ]; then
    ...
    if [ -n "$BENCHMARKDIR" ]; then
        BENCHMARK_ITEMS=($(find "${BENCHMARKDIR}" -type f))
    elif [ -n "$BENCHMARKURLFILE" ]; then
        BENCHMARK_ITEMS=($(cat "${BENCHMARKURLFILE}"))
    else
        echo "Error: failed benchmark or evaluation input source, abort"
        exit 1
    fi
    ...
    for benchmarkitem in "${BENCHMARK_ITEMS[@]}"; do
        ...
        # obtain next benchmark item
        if [[ $benchmarkitem == http*://* ]]; then
            DOWNLOAD_STATUS=0
            wget "$benchmarkitem" -O "$TMP_INPUT_FILE" &> "$WGET_LOG_FILE" || DOWNLOAD_STATUS=$?
            if [ "$DOWNLOAD_STATUS" -ne 0 ]; then
                echo "Error: failed to download benchmark item $benchmarkitem, skipping"
                ls -l "$TMP_INPUT_FILE"
                echo "Download failure:"
                cat "$WGET_LOG_FILE"
                continue
            fi
            benchmark="$(realpath "$TMP_INPUT_FILE")"
        else
        ...
        for solver in "$@"; do
            ...
            if [ -n "$SUSPENDFILE" ]; then
                if [ -r "$SUSPENDFILE" ]; then
                    echo "Checking suspendfile '$SUSPENDFILE' ..."
                    HALT_SECONDS=$(cat "$SUSPENDFILE")
                    if [ -n "$HALT_SECONDS" ]; then
                        echo "Sleeping for $HALT_SECONDS seconds ..."
                        sleep $HALT_SECONDS
                    else
                        echo "Failed to extract wait seconds from suspendfile '$SUSPENDFILE'"
                    fi
                    echo "... halt benchmark due to existing stopfile, and removing it"
                    echo "Deleting suspendfile '$SUSPENDFILE'"
                    rm -f "$SUSPENDFILE"
                fi
            fi
...
echo "Summary" | tee $APPEND "$OUTPUT_FILE"
for solver in "$@"; do
    echo "$solver: par2: ${PAR2["$solver"]} maxtime: ${MAXTIME["$solver"]} solved: ${SOLVED["$solver"]} (sumTime: ${TOTALTIME["$solver"]} sumCPU: ${TOTALCPUTIME["$solver"]} sat: ${SAT["$solver"]} (${TIME_SAT["$solver"]} s) unsat: ${UNSAT["$solver"]} (${TIME_UNSAT["$solver"]} s)) errors: ${ERRORS["$solver"]} (full benchmark: $TRIED_BENCHMARK)"
