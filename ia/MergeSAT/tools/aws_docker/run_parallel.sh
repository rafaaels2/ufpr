...
#   --network mergesat-test // use the Docker bridge network called 'mergesat-test'
...
docker run -i --shm-size=4g --name $NODE_TYPE --network $DOCKER_NETWORK --entrypoint bash --rm -v $HOST_RUNDIR:$DOCKER_RUNDIR -t $DOCKER_IMAGE -c "/competition/init_solver.sh; exec bash"